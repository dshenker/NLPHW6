...
class TaggedCorpus:
    ...
    def __init__(self, *files: Path,
        ...
        if tagset is None or vocab is None:
            ...
            logging.info(f"Read {sum(word_counts.values())} tokens from {', '.join(file.name for file in files)}")
        ...
        if tagset is None:
            logging.info(f"Created {len(self.tagset)} tag types")
        else:
            self.tagset = tagset

        if vocab is None:
            logging.info(f"Created {len(self.vocab)} word types")
        else:
            self.vocab = vocab
    ...
    def get_sentences(self) -> Iterable[Sentence]:
        ...
        for word, tag in self.get_tokens():
            ...
            if word == EOS_WORD:
                yield sentence
                sentence = [(BOS_WORD, BOS_TAG)]   # reset for the next sentence (if any)
...
def is_supervised(sentence: Sentence) -> bool:
    ...
    return all(tag is not None for _, tag in sentence)
...
def desupervise(sentence: Sentence) -> Sentence:
    ...
    return [(word,tag if tag==BOS_TAG or tag==EOS_TAG else None) 
